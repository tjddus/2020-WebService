...
|페이지|기능설명|
|------|---|
|메인페이지|username, password로 로그인 후 상품을 펀딩 및 심사 가능하다 <br> token을 통한 로그인 유지를 진행한다 <br> 상품 전체 정보를 볼 수 있다|
|상품정보페이지|상품의 자세한 정보를 볼 수 있다. (이름, 카테고리, 금액, 좋아요) <br> 상품의 댓글을 추가/삭제/수정 가능하다. <br> 해당 페이지는 심사가 완료된 상품만 볼 수 있다.|
|사용자정보페이지|회원가입, 로그인, 로그아웃이 가능하다. <br> 본인이 펀딩한 상품 및 실제 심사한 상품을 조회가능하다. <br>|
|상품시범단평가|심사 기간 중에 등록된 사용자 중 10명을 랜덤으로 지정해 상품에 대한 평점을 매길 수 있다. <br> 10명의 사용자들의 평점을 바탕으로 등급을 정해 D(5점 미만), F(3점 이하)인 경우 펀딩 상품이 자동 삭제 된다.|
|상품모니터링페이지|관리자를 고려하여 해당 페이지를 구성한 것이다. 꾸준한 모니터링 서비스를 제공함으로써 상품에 대한 품질을 높일 수 있다|
